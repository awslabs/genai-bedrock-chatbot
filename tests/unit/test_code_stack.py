...
STACK = CodeStack(
    core.App(
        context={
            "logging": {"lambda_log_level": "INFO", "streamlit_log_level": "INFO"},
        }
    ),
    "code",
)
    ...
    template.has_resource_properties(
        "AWS::S3::Bucket",
        {
            "BucketName": {
                "Fn::Join": [
                    "",
                    [Match.any_value(), "-sagemaker-pricing-", Match.any_value()],
                ]
            }
        },
    )
    template.has_resource_properties(
        "AWS::S3::Bucket",
        {
            "BucketName": {
                "Fn::Join": [
                    "",
                    [Match.any_value(), "-kendra-data-source-", Match.any_value()],
                ]
            }
        },
    )
...
def test_kendra_index_created():
    ...
    template.has_resource_properties(
        "AWS::Kendra::Index", Match.object_like({"Edition": "DEVELOPER_EDITION"})
    )
    template.has_resource_properties(
        ...
        Match.object_like(
            {
                "DataSourceConfiguration": {
                    "S3Configuration": {"BucketName": Match.any_value()}
                }
            }
        ),
    )
        ...
        Match.object_like(
            {
                ...
                "Timeout": 900,
                "Architectures": ["arm64"],
                "FunctionName": {"Fn::Join": ["", [Match.any_value(), "-chat-lambda"]]},
            }
        ),
    )
...
def test_database_created():
    ...
    template.has_resource_properties(
        ...
        Match.object_like(
            {
                "DatabaseInput": {
                    "Name": {"Fn::Join": ["", [Match.any_value(), "-pricing-db"]]}
                }
            }
        ),
    )
    template.has_resource_properties(
        ...
        Match.object_like(
            {
                "Name": {
                    "Fn::Join": ["", [Match.any_value(), "-sagemaker-pricing-crawler"]]
                }
            }
        ),
    )
